...
def built_LMAMPC_parser():
    ...
    if mode == 'testing':
        test_dir = 'results/Pendulum/LMAMPC-v2-2021-11-24-14-04-57'
    ...
    # env
    parser.add_argument('--env_id', default='Pendulum-v1')
    ...
    parser.add_argument('--con_dim', type=int, default=20)
    ...
    # IO
    time_now = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    results_dir = './results/Pendulum/{algo}-{time}'.format(algo=parser.parse_args().alg_name, time=time_now)
