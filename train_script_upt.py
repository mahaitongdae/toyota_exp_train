...
def built_LMAMPC_parser():
    ...
    parser.add_argument('--mu_lr_schedule', type=list, default=[3e-6, 20000, 1e-6])
    ...
    parser.add_argument('--mu_out_bias', type=float, default=1.)
    ...
    # IO
    time_now = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
    results_dir = './results/uppep_triangle/{algo}-{time}'.format(algo=parser.parse_args().alg_name, time=time_now)
